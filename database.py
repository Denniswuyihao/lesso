...
def _infer_price_base_subcategory(material_desc: object, fallback: object = "") -> str:
    """Build a useful product family from a base-price material description.

    Example:
    直通(美标PVC SCH40给水配件)白色 1＂ -> 直通(美标PVC SCH40给水配件)
    防臭密封套(PVC-U排水配件)白色 dn50×32 -> 防臭密封套(PVC-U排水配件)
    """
    text = clean_scalar(material_desc)
    if not text:
        return _clean_subcategory(fallback)
    s = re.sub(r"\s+", " ", text).strip()
    s = re.sub(r"(白色|灰色|绿色|蓝色|黑色|透明色|本色|红色|黄色)", "", s).strip()
    # Remove common trailing specifications/sizes so the dropdown is not polluted by every size.
    s = re.sub(r"\s*(dn|DN)\s*\d+(?:\s*[×xX*]\s*\d+)*\s*$", "", s).strip()
    s = re.sub(r"\s*D\s*\d+(?:\.\d+)?\s*$", "", s, flags=re.IGNORECASE).strip()
    s = re.sub(r"\s*φ\s*\d+(?:\.\d+)?\s*$", "", s).strip()
    s = re.sub(r"\s*\d+(?:\.\d+)?\s*(?:mm|MM|cm|CM|m|M|米|寸|吋|＂|\")\s*$", "", s).strip()
    s = re.sub(r"\s+", " ", s).strip(" -_/，,;；")
    return _clean_subcategory(s) or _clean_subcategory(fallback)
...
def expand_usd_price_base_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, r in df.iterrows():
        ...
        warehouse_category = clean_scalar(_get_col(r, "仓位描述", "定价小组分类", "category")) or "Price Base"
        size = _extract_size_from_text(material_desc)
        product_type = _infer_product_type(warehouse_category, material_desc)
        subcategory = _infer_price_base_subcategory(material_desc, warehouse_category)

        rows.append({
            "sap": sap,
            "product_type": product_type,
            "subcategory": subcategory,
            "category": _format_category(product_type, subcategory, fallback=warehouse_category),
...
def read_product_import_file(file) -> pd.DataFrame:
    ...
    if detail_frames:
        ...
        if base_price_frames:
            ...
            # v4 change: keep unmatched rows from USD base-price sheets too.
            # Otherwise SAP codes that exist only in 2507美金基准表 can be imported
            # successfully from the user's perspective, but exact SAP search returns 0.
            detailed_saps = set(detailed["sap"].astype(str)) if "sap" in detailed.columns else set()
            extra_base = base_prices[~base_prices["sap"].astype(str).isin(detailed_saps)].copy()
            if not extra_base.empty:
                detailed = combine_imported_product_frames([detailed, extra_base])
        return detailed
...
def clean_products_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["active"] = pd.to_numeric(df["active"], errors="coerce").fillna(1).round().astype(int)
    df["active"] = df["active"].apply(lambda x: 1 if int(x) != 0 else 0)
...
# -------- 客户询价/报价草稿导入：SAP/型号/数量 -> 数据库匹配预览 --------
QUOTE_REQUEST_ALIASES={"sap":"input_sap","sap号":"input_sap","SAP号":"input_sap","sap no.":"input_sap","SAP No.":"input_sap","物料":"input_sap","物料编码":"input_sap","编号":"input_sap","型号":"input_model","model":"input_model","Model":"input_model","品名型号":"input_model","description":"input_description","描述":"input_description","品名":"input_description","数量":"requested_quantity","qty":"requested_quantity","Qty":"requested_quantity","quantity":"requested_quantity","Quantity":"requested_quantity","单位":"input_unit","unit":"input_unit"}

def _find_quote_header_row(raw:pd.DataFrame)->int|None:
    kws=["sap","sap号","物料","型号","model","数量","qty","品名型号"]
    best=(-1,None)
    for i in range(min(len(raw),30)):
        s="|".join([clean_scalar(x).lower() for x in raw.iloc[i].tolist() if clean_scalar(x)])
        score=sum(1 for k in kws if k.lower() in s)
        if score>best[0]: best=(score,i)
    return best[1] if best[0]>=2 else None

def _normalize_quote_cols(df:pd.DataFrame)->pd.DataFrame:
    mp={}
    for c in df.columns:
        raw=str(c).strip(); mp[c]=QUOTE_REQUEST_ALIASES.get(raw) or QUOTE_REQUEST_ALIASES.get(raw.lower()) or raw
    df=df.rename(columns=mp).copy()
    for c in ["input_sap","input_model","input_description","requested_quantity","input_unit"]:
        if c not in df.columns: df[c]=None
    return df

def read_quote_request_file(file)->pd.DataFrame:
    name=str(getattr(file,"name","")).lower(); frames=[]
    if name.endswith('.csv'):
        df=_normalize_quote_cols(pd.read_csv(file)); df['source_sheet']='CSV'; df['source_row']=df.index+2; frames=[df]
    else:
        sheets=pd.read_excel(file,sheet_name=None,header=None)
        for sh,raw in sheets.items():
            hr=_find_quote_header_row(raw)
            if hr is None:
                rows=[]
                for ridx,row in raw.iterrows():
                    text=' '.join(clean_scalar(x) for x in row.tolist() if clean_scalar(x))
                    for sap in re.findall(r"\b\d{6,20}\b",text): rows.append({'input_sap':sap,'requested_quantity':1,'source_sheet':sh,'source_row':ridx+1})
                part=pd.DataFrame(rows)
            else:
                try: file.seek(0)
                except Exception: pass
                part=_normalize_quote_cols(pd.read_excel(file,sheet_name=sh,header=hr)); part['source_sheet']=str(sh); part['source_row']=part.index+hr+2
            if not part.empty: frames.append(part)
    df=pd.concat(frames,ignore_index=True) if frames else pd.DataFrame()
    if df.empty: return pd.DataFrame(columns=['input_sap','input_model','input_description','requested_quantity','input_unit','source_sheet','source_row'])
    for c in ['input_sap','input_model','input_description','input_unit']:
        if c not in df.columns: df[c]=''
        df[c]=df[c].fillna('').apply(clean_scalar)
    if 'requested_quantity' not in df.columns: df['requested_quantity']=1
    df['requested_quantity']=pd.to_numeric(df['requested_quantity'].apply(clean_scalar),errors='coerce').fillna(1)
    df.loc[df['requested_quantity']<=0,'requested_quantity']=1
    df=df[(df['input_sap']!='')|(df['input_model']!='')|(df['input_description']!='')].copy()
    df=df[~df['input_sap'].astype(str).str.contains('合计|total',case=False,na=False)].copy()
    df['input_sap']=df['input_sap'].apply(lambda x:(re.findall(r"\b\d{6,20}\b",str(x)) or [clean_scalar(x)])[0] if clean_scalar(x) else '')
    df['_key']=df.apply(lambda r:r['input_sap'] or (r['input_model'] or r['input_description']).lower(),axis=1)
    return df.groupby('_key',as_index=False).agg({'input_sap':'first','input_model':'first','input_description':'first','requested_quantity':'sum','input_unit':'first','source_sheet':'first','source_row':lambda x:', '.join(map(str,x))[:120]}).drop(columns=['_key'],errors='ignore')

def _match_score(q:str,row:pd.Series)->int:
    q=clean_scalar(q).lower(); hay=' | '.join(clean_scalar(row.get(f)).lower() for f in ['model','size_mm','cn_name','en_name','description','material_description','subcategory','category'])
    if not q: return 0
    score=100 if q==clean_scalar(row.get('model')).lower() else 0
    if q in hay: score+=40
    score+=sum(3 for t in re.split(r"\W+",q) if len(t)>=2 and t in hay)
    return score

def build_quote_request_preview(request_df:pd.DataFrame)->pd.DataFrame:
    init_db(); req=request_df.copy() if request_df is not None else pd.DataFrame()
    if req.empty: return pd.DataFrame()
    products=load_products(limit=None,only_active=True)
    rows=[]
    if products.empty:
        req['match_status']='数据库为空'; return req
    products['sap_norm']=products['sap'].astype(str).str.lower().str.strip(); bysap={r['sap_norm']:r for _,r in products.iterrows()}
    for _,r in req.iterrows():
        insap=clean_scalar(r.get('input_sap')); q=clean_scalar(r.get('input_model')) or clean_scalar(r.get('input_description')); qty=numeric_value(r.get('requested_quantity'),1) or 1
        m=None; status='未匹配'; reason='没有找到对应 SAP/型号'
        if insap and insap.lower() in bysap: m=bysap[insap.lower()]; status='已匹配-SAP'; reason='按 SAP 精确匹配'
        elif q:
            scored=sorted([(_match_score(q,p),p) for _,p in products.iterrows() if _match_score(q,p)>0],key=lambda x:x[0],reverse=True)
            if scored: m=scored[0][1]; status='已匹配-型号/描述' if scored[0][0]>=60 else '需人工确认-多候选'; reason=f'按型号/描述匹配，分数 {scored[0][0]}'
        base={'select':status.startswith('已匹配'),'match_status':status,'match_reason':reason,'input_sap':insap,'input_model':clean_scalar(r.get('input_model')),'input_description':clean_scalar(r.get('input_description')),'requested_quantity':qty,'input_unit':clean_scalar(r.get('input_unit')),'source_sheet':clean_scalar(r.get('source_sheet')),'source_row':clean_scalar(r.get('source_row'))}
        for c in PRODUCT_COLUMNS: base[c]=m.get(c) if m is not None else ('' if c in TEXT_COLUMNS else 0)
        rows.append(base)
    prev=pd.DataFrame(rows); matched=prev[prev['sap'].astype(str).str.strip()!=''].copy(); un=prev[prev['sap'].astype(str).str.strip()==''].copy()
    if not matched.empty:
        agg={c:'first' for c in matched.columns if c!='requested_quantity'}; agg['requested_quantity']='sum'; matched=matched.groupby('sap',as_index=False).agg(agg)
    return pd.concat([matched,un],ignore_index=True)
