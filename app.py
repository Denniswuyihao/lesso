...
def make_export_signature(items: pd.DataFrame, customer: str, quote_no: str, price_note: str) -> str:
    cols = ["sap", "product_type", "subcategory", "category", "cn_name", "en_name", "model", "description", "color", "size_mm", "weight", "material_description", "quote_price", "quantity", "amount", "stock", "packing_volume", "qty_per_ctn", "total_volume", "unit", "package_info", "image_url"]
    safe = items.copy()
    for col in cols:
...
with st.expander("上传客户询价表 / 旧报价单并预览", expanded=False):
    ...
    if st.button("读取并生成预览", use_container_width=True) and request_file is not None:
        try:
            ...
            st.success(f"已生成预览：{len(st.session_state['quote_request_preview'])} 行。")
        except Exception as exc:
            st.error(f"读取客户文档失败：{exc}")
    preview_df = st.session_state.get("quote_request_preview")
    if isinstance(preview_df, pd.DataFrame) and not preview_df.empty:
        show_cols=["select","match_status","match_reason","input_sap","input_model","input_description","requested_quantity","sap","product_type","subcategory","category","cn_name","model","price","unit","stock","packing_volume","qty_per_ctn"]
        for col in show_cols:
            if col not in preview_df.columns: preview_df[col]=""
        edited_preview=st.data_editor(preview_df[show_cols], hide_index=True, use_container_width=True, height=320, disabled=[c for c in show_cols if c not in ["select","requested_quantity"]], column_config={"select":st.column_config.CheckboxColumn("加入",width="small"),"requested_quantity":st.column_config.NumberColumn("数量",min_value=0.0,step=1.0,format="%.4f")}, key="quote_request_preview_editor")
        if st.button("把勾选且已匹配的产品加入报价购物车", type="primary", use_container_width=True):
            ...
            if selected.empty: st.warning("没有可加入的已匹配产品。")
                ...
                for _,row in selected.iterrows():
                    sap_key=clean_text(row.get("sap")); item=preview_df[preview_df["sap"].astype(str)==sap_key].iloc[0].to_dict(); new_qty=qty_number(row.get("requested_quantity"),1.0)
                    ...
                    else: item["quantity"]=new_qty; st.session_state.cart[sap_key]=item
                reset_export_cache(); st.success(f"已加入/合并 {len(selected)} 个 SKU 到购物车。")
...
products = cached_products(keyword, sap_text, product_type, subcategory, category, min_price, max_price, min_stock, has_image, int(limit))
...
if products.empty:
    ...
    with results_col:
        ...
        display_cols = ["select", "image_preview", "sap", "product_type", "subcategory", "category", "cn_name", "model", "color", "size_mm", "description", "price", "stock", "packing_volume", "qty_per_ctn", "unit", "quantity"]
        ...
        with st.form("product_select_form"):
            edited_products = st.data_editor(
                ...
                height=430,
                disabled=["image_preview", "sap", "product_type", "subcategory", "category", "cn_name", "model", "color", "size_mm", "description", "price", "stock", "packing_volume", "qty_per_ctn", "unit"],
                column_config={
...
with st.form("cart_form"):
    edited_cart = st.data_editor(
        cart_editor[["remove", "sap", "product_type", "subcategory", "category", "cn_name", "model", "color", "size_mm", "price", "quantity", "stock", "packing_volume", "qty_per_ctn", "unit", "package_info"]],
        hide_index=True,
        use_container_width=True,
        height=260,
        disabled=["sap", "product_type", "subcategory", "category", "cn_name", "model", "color", "size_mm", "price", "stock", "packing_volume", "qty_per_ctn", "unit", "package_info"],
        column_config={
...
preview_cols = ["sap", "product_type", "subcategory", "category", "cn_name", "model", "color", "size_mm", "weight", "quote_price", "quantity", "amount", "stock", "packing_volume", "qty_per_ctn", "total_volume", "unit", "package_info"]
