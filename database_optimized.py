...
def read_product_import_file(file) -> pd.DataFrame:
    ...
    2. PPR wide quotation table with Grey SAP No. / Green SAP No.
    3. PVC-U drainage wide quotation table with 4m / 6m / 5.8m SAP columns.
    4. USD base price table with 物料 / 物料描述 / 单价.
    """
    ...
    if not frames:
        ...
        raise ValueError(
            "未识别到可导入的产品数据。支持：标准字段、PPR宽表、PVC-U排水宽表、USD基准表。"
            + (f" 工作表检查结果：{detail}" if detail else "")
        )

    return combine_imported_product_frames(frames)
...
def clean_products_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "sap" not in df.columns:
        raise ValueError("导入文件必须包含 SAP 号列，或包含 PPR 宽表字段 Grey SAP No. / Green SAP No.，或包含 PVC-U 排水宽表字段，或包含 USD 基准表字段 物料 / 物料描述 / 单价。")
